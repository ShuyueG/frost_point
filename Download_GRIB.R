...
library(RCurl)
Download_GRIB<-function(year,day,time){
# -----time check--------
  legal_time<-c("00","03","06","09","12","15","18","21")
  if (is.na(match(time,legal_time))){ # time is legal or not
    return(paste(time,"is not a legal time. Legal time must be in [00,03,06,09,12,15,18,21]")) # not legal
  }
# ------- Url check---------
  web<-"ftp://hydro1.sci.gsfc.nasa.gov/data/s4pa/GLDAS_V1/GLDAS_NOAH025SUBP_3H"
  Url<-paste(web,"/",year,"/",day,"/",sep="")
  if (!url.exists(Url)){ # url is real or not
...
# -------- download the file----------
    dat <-
      getBinaryURL(
        paste(Url,down_name,sep="")
      )
    writeBin(dat, paste("c:/data/",down_name,sep="")) # download file in "c:/data/"
