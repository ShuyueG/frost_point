...
Extract_GRIB <- function(year, day, time) {
  ...
  result<-Download_GRIB(year, day, time)
  fname<-substr(result,1,56)# get the file's name
  
  Tt <- grb_extr(fname, 24) # get temperature (in K)
  q<-grb_extr(fname, 25) # get Specific humidity (in kg/kg)
  p<-grb_extr(fname, 26) # get surface air pressure (in Pa)
  # compute dew point
  dew<-matrix(data=NA,nrow=1440,ncol=600) # new matrix of dew point
  for (i in 1:1440){
    for (j in 1:600){
      a_p<-p[i,j]
      a_q<-q[i,j]
      if ((!is.na(a_p))&(!is.na(a_q))){ # both p and q are not NA
        dew[i,j]<-dew_point(a_p*0.01,a_q) # 1 Pa=0.01 mb
      }
    }
  }
  # store data in Rdata file
  save(Tt,q,p,dew,file=paste(substr(fname,1,20),"_",year,"_",day,"_",time,".Rdata",sep=""))
